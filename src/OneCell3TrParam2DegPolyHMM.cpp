...
void OneCell3TrParam2DegPolyHMM::print(FILE* stream) {
  ...
  //fprintf(stream, "BEFORE manual set, ll: %0.20f\n", this->getLogLikelihood());
  //// manual set for checking
  //gsl_vector* estParams = gsl_vector_alloc(4);
  //gsl_vector_set(estParams, 0, 0.910672605);
  //gsl_vector_set(estParams, 1, 0.004162637);
  //gsl_vector_set(estParams, 2, 724.9786168);
  //gsl_vector_set(estParams, 3, 0.005369434);
  //this->setParamsToEst(estParams);
  //fprintf(stream, "AFTER manual set, ll: %0.20f\n", this->getLogLikelihood());
...
double OneCell3TrParam2DegPolyHMM::getTotalEmissionProb(int stateIdx, std::vector<std::vector<double>*>* currChrDepthsVec, int chrIdx, int depthIdx) {
  ...
  //return this->getEmissionProb(currTumorDepth, currDiploidDepth, currPloidy, 0);
}
 ...
 * Always saves in order of [lib, alpha, beta, lambda, t]
 */
//void OneCell3TrParam2DegPolyHMM::saveParamEstimates(std::string filename) const {
//  std::ofstream outFile(filename);
//  outFile << std::fixed << std::setprecision(40);
//  double beta = gsl_vector_get(this->paramsToEst, this->TRANSITION_PROB_START_IDX + 0);
//  double lambda = gsl_vector_get(this->paramsToEst, this->TRANSITION_PROB_START_IDX + 1);
//  double t = gsl_vector_get(this->paramsToEst, this->BRANCH_LENGTH_START_IDX);
//
//  outFile << this->getLibScalingFactor(0) << std::endl;
//  outFile << this->getAlpha() << std::endl;
