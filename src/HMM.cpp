...
//void HMM::allocIntermediates() {
  ...
  //this->extractedTrBranchParams = gsl_vector_alloc(this->NUM_TRANSITION_PARAMS_TO_EST + this->NUM_BRANCH_LENGTHS_TO_EST);
  ...
  //this->numTransitionsMat = nullptr;
...
HMM::~HMM() {
  ...
  gsl_matrix_complex_free(this->ssEvec);
  gsl_matrix_free(this->transitionTranspose);
  //gsl_vector_free(this->extractedTrBranchParams);
...
double HMM::setTransition() {
  ...
  for(int i = 0; i < this->NUM_BRANCH_LENGTHS_TO_EST; i++) {
    //std::cout << "from: " << BRANCH_LENGTH_START_IDX + i  << " to: " << extractedTrBranchParamsIdx << std::endl;
  ...
  //printRowVector(extractedTrBranchParams);
...
double HMM::runForwardAlg() {
  ...
  for(unsigned int chrIdx = 0; chrIdx < chrVec->size(); chrIdx++) {
    ...
    for(stateIdx = 0; stateIdx < (int) this->initProb->size; stateIdx++) {
      ...
      //currResPloidy = emissionProb + log(gsl_vector_get(this->initProb, stateIdx)); // extra exp/log?
      currResPloidy = emissionProb * (gsl_vector_get(this->initProb, stateIdx));

      //gsl_matrix_set(forwardMat, stateIdx, 0, exp(currResPloidy)); // extra exp/log?
    ...
    for(i = 1, depthIdx = 1; i < forwardMat->size2; i++, depthIdx++) {
      ...
      for(stateIdx = 0; stateIdx < (int) this->states->size(); stateIdx++) {
        ...
        for(j = 0; j < this->prevForwardCol->size; j++) {
          ...
          //gsl_vector_set(this->currForwardCol, stateIdx, currResPloidy + exp(emissionProb + log(gsl_vector_get(this->prevForwardCol, j)) + log(gsl_matrix_get(this->transition, j, stateIdx)))); // extra exp/log?
...
double HMM::runBackwardAlg() {
  ...
  for(unsigned int chrIdx = 0; chrIdx < chrVec->size(); chrIdx++) {
    ...
    //for(i = backwardMat->size2 - 1, depthIdx = (*firstDepthPair->regions)[currChr]->size() - 1; i >= 0 ; i--, depthIdx--) { // -1 would get last col
      ...
      for(stateIdx = 0; stateIdx < (int) this->states->size(); stateIdx++) {
        ...
        for(j = 0; j < (int) this->nextBackwardCol->size; j++) {
          ...
          //emissionProb = this->getTotalLogEmissionProb(j, currChrDepthsVec, chrIdx, depthIdx); // extra exp/log?
          ...
          //gsl_vector_set(this->currBackwardCol, stateIdx, currResPloidy + exp(emissionProb + log(gsl_vector_get(this->nextBackwardCol, j)) + log(gsl_matrix_get(this->transition, stateIdx, j)))); // extra exp/log?
          gsl_vector_set(this->currBackwardCol, stateIdx, currResPloidy + emissionProb * gsl_vector_get(this->nextBackwardCol, j) * gsl_matrix_get(this->transition, stateIdx, j));
          //double e1 = log(emissionProb);
          //double e2 = exp(e1);
          //double e3 = log(e2);
          //double e4 = exp(e3);
          //gsl_vector_set(this->currBackwardCol, stateIdx, currResPloidy + exp(log(e4) + log(gsl_vector_get(this->nextBackwardCol, j)) + log(gsl_matrix_get(this->transition, stateIdx, j)))); // explogtest
        }
      }
    ...
    for(stateIdx = 0; stateIdx < (int) this->initProb->size; stateIdx++) {
      ...
      //emissionProb = this->getTotalLogEmissionProb(stateIdx, currChrDepthsVec, chrIdx, 0); // extra exp/log?
      emissionProb = this->getTotalEmissionProb(stateIdx, currChrDepthsVec, chrIdx, 0);
      ...
      //currRes = exp(emissionProb + log(initProb) + log(gsl_matrix_get(backwardMat, stateIdx, 0)) - log(scalingFactor)); // Wed 17 Jun 2020 03:13:29 PM PDT changed back to 1:1 mapping of times and indices // extra exp/log?
      currRes = emissionProb * initProb * gsl_matrix_get(backwardMat, stateIdx, 0) / scalingFactor; // Wed 17 Jun 2020 03:13:29 PM PDT changed back to 1:1 mapping of times and indices
  ...
  for(unsigned int chrIdx = 0; chrIdx < chrVec->size(); chrIdx++) {
    ...
    for(i = 1, depthIdx = 1; i < forwardMat->size2; i++, depthIdx++) {
      ...
      for(stateIdx = 0; stateIdx < (int) this->states->size(); stateIdx++) {
        ...
        //emissionProb = this->getTotalLogEmissionProb(stateIdx, currChrDepthsVec, chrIdx, depthIdx); // extra exp/log?
        emissionProb = this->getTotalEmissionProb(stateIdx, currChrDepthsVec, chrIdx, depthIdx);
        ...
        for(j = 0; j < this->prevForwardCol->size; j++) {
          ...
          //currResPloidy = gsl_vector_get(this->currForwardCol, stateIdx) + exp(emissionProb) * gsl_vector_get(this->prevForwardCol, j) * gsl_matrix_get(this->transition, j, stateIdx); // extra exp/log
          currResPloidy = gsl_vector_get(this->currForwardCol, stateIdx) + (emissionProb) * gsl_vector_get(this->prevForwardCol, j) * gsl_matrix_get(this->transition, j, stateIdx);
      ...
      // save this->currForwardCol and scalingFactor
      gsl_matrix_set_col(forwardMat, i, this->currForwardCol);
      //printMatrix(stderr, forwardMat);
      //printMatrix(stderr, backTrace);
    }
    ...
    for(cellIdx = 0; cellIdx < this->NUM_CELLS; cellIdx++) {
      ...
      //  currVitPathMap->erase(currChr);
      //}
      //(*currVitPathMap)[currChr] = new std::vector<int>(std::begin(paths[cellIdx]), std::end(paths[cellIdx]));
      if(currVitPathMap->count(currChr) == 0) {
...
double HMM::checkForInitProbGaps() {
  // Tue 16 Nov 2021 12:16:11 PM PST
  //// check if the initProb vector is something like [0.15, 0, 0.8, 0, 0.05, 0], where the max indices are multiples of each other
  // check if initProb vector has 0 for idx 1 and 2
  //if(this->MAX_PLOIDY >= 2 && compareDoubles(0, gsl_vector_get(this->initProb, 1)) && compareDoubles(0, gsl_vector_get(this->initProb, 2))) {
  //  return GSL_NAN;
  //}
